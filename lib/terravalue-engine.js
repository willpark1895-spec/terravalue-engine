...
class EcosystemServices {
  ...
  static calculate(parcel) {
    ...
    // Property premium — a ONE-TIME capitalized uplift on market value (a stock),
    // capped at maxPremiumPct (12%). It is NOT a per-year flow, so it is deliberately
    // kept OUT of totalAnnual and reported separately as propertyPremiumOneTime below.
    // (Stock-vs-flow separation — Option A, 2026-06-08.)
    ...
    // Rounded to 12dp: enough to clear IEEE 754 noise (0.07 * 45/30 is
    // 0.10500000000000001) without moving any dollar figure. 6dp would have shifted
    // three golden fixtures by $1 where the rate repeats — measured 2026-09-11.
    const appliedPremiumPct = Math.round(cappedPremiumPct * 1e12) / 1e12;
    const propertyPremium = Math.round(
      marketValue * appliedPremiumPct
      // Scaled linearly — 30% canopy = full 7% premium (Siriwardena optimal), capped at 12%
    );

    // totalAnnual = the five genuine per-year ecosystem service flows ONLY.
    // The one-time property premium is excluded to keep the units consistent (all $/yr).
    const totalAnnual = carbon + stormwater + airQuality + energy + habitat;
    ...
    return {
      services: {
        ...
        // Retained for the breakdown display and backward compatibility, but flagged
        // one-time so it is never re-summed into the annual total or shown as $/yr.
        // premiumPct reports the rate that ACTUALLY produced `value`, not the config
        // constant. Until 1.6.0 this spread the unscaled 0.07 next to a value computed
        // at up to 12%, so `value` could not be reproduced from the payload — found
        // 2026-09-11 against the live /api/ecosystem response. The literature rate is
        // still published, as basePremiumPct.
        propertyPremium: {
          value: propertyPremium,
          oneTime: true,
          ...rates.propertyPremium,
          premiumPct: appliedPremiumPct,
          basePremiumPct: rates.propertyPremium.premiumPct,
          premiumScaling: 'linear in canopyPct / 30, capped at maxPremiumPct',
          premiumCapped: rawPremiumPct > CANOPY_VALUE_COEFFICIENTS.maxPremiumPct / 100,
        },
      },
      totalAnnual,
      // One-time capitalized property-value uplift (a stock), surfaced separately from
      // the recurring annual flows in totalAnnual. See the propertyPremium note above.
      propertyPremiumOneTime: propertyPremium,
      parcelMetrics: {
      ...
      methodology: METHODOLOGY_VERSIONS.ecosystem,
    };
  }
...
class LandAppreciation {
  ...
  static project(params) {
    ...
    return {
      ...
      methodology: {
        version: METHODOLOGY_VERSIONS.appreciation,
...
class Methodology {
  ...
  static generate() {
    return {
      ...
      // Which module actually moved. The umbrella version above cannot say.
      moduleVersions: METHODOLOGY_VERSIONS,
...
class LandValuation {
  ...
  static fullValuation(parcel, options = {}) {
    ...
    return {
      ...
      version: METHODOLOGY_VERSIONS.landValuation,
      ...
      ecosystemServices: {
        annualValue: ecoServices.totalAnnual,                       // recurring annual flows only
        services: ecoServices.services,
        propertyPremiumOneTime: ecoServices.propertyPremiumOneTime, // one-time uplift, not capitalized again
  ...
  static _getZoningUses(zoning) {
    ...
    // ⚠ ORDER MATTERS: this test MUST precede the M/I/LI/HI industrial test
    // below. 'MU' and 'MX' both start with 'M', so while industrial came
    // first this branch was unreachable and every mixed-use district — the
    // calculator's own 'MU (Mixed Use)' option included — silently returned
    // the industrial use set with residential dropped. No error, no NaN, no
    // test failure. Found 2026-08-27 while mapping live municipal zoning.
...
// ============================================================
// SOIL SCORE MODULE (v2) — config-weighted composite (0–100)
// ============================================================

class SoilScore {
  /**
   * Soil Score v2 — weighted composite of normalized environmental sub-scores.
   *
   * @param {Object} inputs — { canopyPct, annualPM25, parkAccessPct, walkabilityIndex, imperviousPct }
   *                          (any subset — missing factors are dropped and the remaining weights renormalized)
   * @param {Object} [opts] — { weights } to override config weights (e.g. set a weight to 0)
   * @returns {Object} { score, confidence, coverage, subScores, absolute, factorsPresent, weightsUsed, version, methodology }
   *
   * Definition-agnostic: weights live in config/soil-score.json, so "pure
   * stewardship" is just zeroing the walkability/parkAccess weights. Weights are
   * renormalized over whichever sub-scores have real inputs; confidence reflects
   * how much of the total weight is backed by real data. The raw inputs are also
   * returned (`absolute`) so cross-city comparison isn't erased by normalization.
   */
  static calculate(inputs = {}, opts = {}) {
    const cfg = SOIL_SCORE_CONFIG;
    const weights = Object.assign({}, cfg.weights, opts.weights || {});
    const defs = cfg.subScores;
    ...
    for (const key of Object.keys(weights)) {
      ...
      const raw = inputs[def.input];
      const num = (raw === null || raw === undefined || raw === '') ? NaN : Number(raw);
      const usable = w > 0 && Number.isFinite(num);

      if (!usable) {
        subScores[key] = {
          score: null, included: false, weight: w, input: def.input,
          value: Number.isFinite(num) ? num : null, label: def.label,
        };
        continue;
      }

      const score = SoilScore._normalize(num, def);
      subScores[key] = { score, included: true, weight: w, input: def.input, value: num, label: def.label };
      weightedSum += score * w;
      weightPresent += w;
      factorsPresent.push(key);
    }

    const composite = weightPresent > 0 ? Math.round(weightedSum / weightPresent) : null;
    const coverage = weightTotal > 0 ? weightPresent / weightTotal : 0;
    const c = cfg.confidence;
    const confidence = composite === null ? 'none'
      : coverage >= c.highCoverage ? 'high'
      : coverage >= c.moderateCoverage ? 'moderate'
      : 'low';
    ...
    return {
      ...
      subScores,
      absolute: SoilScore._absolutes(inputs, defs),
      factorsPresent,
      weightsUsed: weights,
      version: cfg._version,
      methodology: `TerraValue Soil Score v${cfg._version} — weighted composite of normalized 0–100 sub-scores; weights renormalized over available factors`,
    };
  }

  /** Map a raw input to a 0–100 sub-score per its config method, clamped to [0,100]. */
  static _normalize(v, def) {
    let s;
    switch (def.method) {
      case 'identity':       s = v; break;
      case 'invertPct':      s = 100 - v; break;
      case 'linearToTarget': s = (v / def.target) * 100; break;
      case 'linearRange':    s = ((v - def.min) / (def.max - def.min)) * 100; break;
      case 'invertRange':    s = (1 - (v - def.good) / (def.bad - def.good)) * 100; break;
      default:               s = v;
    }
    return Math.max(0, Math.min(100, Math.round(s)));
  }

  /** Raw, un-normalized metrics — kept for cross-city (absolute) comparison. */
  static _absolutes(inputs, defs) {
    const out = {};
    for (const key of Object.keys(defs)) {
      const inp = defs[key].input;
      const raw = inputs[inp];
      const num = (raw === null || raw === undefined || raw === '') ? NaN : Number(raw);
      if (Number.isFinite(num)) out[inp] = num;
    }
    return out;
  }
}
...
class TerraValueEngine {
  ...
  static SoilScore = SoilScore;
  static SOIL_SCORE_CONFIG = SOIL_SCORE_CONFIG;
}
