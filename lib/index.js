...
// BROWSER-STRIP-START — FittedValuation is server-side only (Gate 1, 2026-07-15).
// Fitted per-market AVM estimator: loads a coefficient artifact from
// config/hedonic/ and scores it. For markets with an artifact this is the AVM
// estimator; salesComparison remains available as the explanation layer.
// Browser bundles exclude it (artifacts load via require; frontends use the API).
const { FittedValuation } = require('./fitted-valuation');
FittedValuation.loadArtifact = function loadArtifact(market) {
  // eslint-disable-next-line global-require, import/no-dynamic-require
  return require(`../config/hedonic/${market}-1.0.json`);
};
TerraValueEngine.FittedValuation = FittedValuation;
// BROWSER-STRIP-END
